...
PCOTを起動した後にこのロガーを実行すると、同じフォルダにあるロガーと同名の.iniファイルを読み込んで起動し常駐する。  
起動後はPCOTの翻訳結果コントロールを監視し、空でないかつ前回チェック時と異なる文字列が表示されている場合にロガーを置いてあるパスにLogsフォルダを作成し、その中にJSONL形式でログを出力する。
